...
SimilarityCalculator::SimilarityCalculator(
    ...
    double ref_bpm,
    double perf_bpm
) : ref_notes(ref), perf_notes(perf), ref_bpm(ref_bpm), perf_bpm(perf_bpm) {}
...
void SimilarityCalculator::perform_fallback_check(
    ...
    for (size_t perf_start = 0; perf_start + 1 < perf_notes.size(); ++perf_start) {
        ...
        while (ref_idx + 1 < ref_notes.size() && perf_idx + 1 < perf_notes.size()) {
            ...
            double ref_duration = ref_notes[ref_idx].note_value;
            ...
            for (size_t search_idx = perf_idx + 1; search_idx < perf_notes.size(); ++search_idx) {
                ...
                std::cout << "Trying ref[" << ref_idx << "] (" << ref_notes[ref_idx].pitch
                          << "->" << ref_notes[ref_idx + 1].pitch << ", interval=" << ref_interval << ") "
                          << "with perf[" << perf_idx << "] (" << perf_notes[perf_idx].pitch
                          << ") to perf[" << search_idx << "] (" << perf_notes[search_idx].pitch
                          << "), interval=" << perf_interval
                          << ", acc_dur=" << accumulated_duration
                          << ", ref_dur=" << ref_duration << std::endl;

                if (perf_interval == ref_interval &&
                    accumulated_duration + absolute_epsilon >= (1.0 - rhythm_tolerance) * ref_duration &&
                    accumulated_duration - absolute_epsilon <= (1.0 + rhythm_tolerance) * ref_duration)
                {
                    std::cout << "✅ Matched ref[" << ref_idx << "] to perf[" << perf_idx
                              << "->" << search_idx << "]\n";
        ...
        if (matched_pairs >= 2) {
            ...
            if (perf_start + matched_length <= static_cast<int>(perf_notes.size())) {
                std::cout << "🎯 Found match: perf_start=" << perf_start
...
std::vector<MatchSegment> SimilarityCalculator::find_similar_segments(double similarity_threshold) {
    ...
    if (!has_high_similarity) {
        perform_fallback_check(candidates, similarity_threshold);
    }

    // 去重和排序逻辑
    std::sort(candidates.begin(), candidates.end(), [](const MatchSegment& a, const MatchSegment& b) {
        if (a.length != b.length) return a.length > b.length; // 长度优先
        return a.similarity > b.similarity; // 其次按相似度
    });
